    ...
    <div data-disclosure-workspace className="pulse-product-page flex flex-1 min-h-0 flex-col bg-background font-sans text-foreground">
      ...
      <header className="shrink-0 border-b border-pl-border px-6 py-2 lg:py-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <p role="status" className="text-xs text-pl-text-3">{saveMessage}</p>
        </div>
          ...
          <div className="grid min-w-0 gap-8 lg:grid-cols-3">
                ...
                {sections.map((s) => {
                  ...
                  return <section key={s.id} ref={(el) => { sectionRefs.current[s.id] = el; }} className="scroll-mt-4 py-2">
                    <button type="button" aria-expanded={open} onClick={() => setOpenSection(open ? "" : s.id)} className="flex w-full items-center justify-between gap-3 rounded-sm py-3 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
                      <span className="text-base font-semibold">{SECTION_TITLES[s.id] || s.title}</span><span className="flex items-center gap-3 text-xs text-pl-text-3">{sectionComplete(s.id) ? "Complete" : sectionHasContent(s.id) ? "In progress" : "Not started"}<ChevronDown className={`size-4 ${open ? "" : "-rotate-90"}`} /></span>
                    </button>
                    {open && <div className="space-y-5 pb-4">{s.questions.map((q: any) => {
                      ...
                      return <div key={q.id}>
                        <div className="flex flex-wrap items-baseline justify-between gap-2"><label htmlFor={`f-${q.id}`} className="text-sm font-medium">{meta.label}{meta.required && <span className="ml-2 font-normal text-pl-text-3">Required</span>}</label>{provenance[q.id] && <span className="text-xs text-pl-text-3">{PROVENANCE_CHIP[provenance[q.id]].label}</span>}</div>
            ...
            <aside className="min-w-0 space-y-5">
              ...
              <section><h2 className="text-sm font-semibold">Optional evaluation</h2><p className="mt-1 text-xs text-pl-text-3">AI-assisted and advisory. You can submit without it.</p>
                {scored ? <><p className="mt-3 text-2xl font-semibold tabular-nums">{score10?.toFixed(1)}<span className="text-sm font-normal text-pl-text-3"> /10</span></p><p className="mt-1 text-sm">{score10! >= 7 ? "Highly novel" : score10! >= 4 ? "Moderately novel" : score10! >= 2 ? "Marginally novel" : "Closely matched"}</p>{dirtySinceScore && <p className="mt-2 text-xs text-pl-amber-text">Evaluated before your latest edits.</p>}<p className="mt-2 text-sm text-pl-text-2">{scoreMeta?.scoringResult?.summary}</p>{strengthenTips.length > 0 && <details className="mt-3"><summary className="cursor-pointer text-sm">How to strengthen</summary><ul className="mt-2 list-disc space-y-2 pl-4 text-sm text-pl-text-2">{strengthenTips.map((tip) => <li key={tip}>{tip}</li>)}</ul></details>}</> : scoringActive ? <div className="mt-3"><EvaluationProgress compact evaluationId={runningEvaluationId} reference={idea?.reference_number} /></div> : <p className="mt-3 text-sm text-pl-text-2">{signal?.headline || signal?.message || "As you write, this signal helps you spot what needs explanation."}</p>}
      ...
      <Dialog open={!!conflictDraft} onOpenChange={(open) => { if (!open) setConflictDraft(null); }}><DialogContent className="max-h-full overflow-y-auto"><DialogHeader><DialogTitle>Compare the saved revision</DialogTitle><DialogDescription>Your answers remain in the disclosure. Review the latest saved answers before choosing which version to keep.</DialogDescription></DialogHeader><div className="space-y-3">{disclosureSections(conflictDraft?.meta_data).map((section) => <details key={section.id}><summary className="cursor-pointer text-sm font-medium">{section.title}</summary>{section.questions.filter((q) => q.answer).map((q) => <p key={q.id} className="mt-2 text-sm">{q.answer}</p>)}</details>)}</div><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => { setSections(disclosureSections(conflictDraft.meta_data)); versionRef.current = conflictDraft.version ?? 0; sessionStorage.removeItem(recoveryKey); setSaveState("saved"); setConflictDraft(null); }}>Use saved revision</Button><Button size="sm" onClick={async () => { versionRef.current = conflictDraft.version ?? 0; try { await saveNow(sections, provenance, completion); setConflictDraft(null); } catch { /* Keep the comparison open on failure. */ } }}>Save my answers instead</Button></div></DialogContent></Dialog>
      <Dialog open={confirmSubmit} onOpenChange={setConfirmSubmit}><DialogContent><DialogHeader><DialogTitle>{requestedChanges ? "Resubmit this disclosure?" : "Submit this disclosure for review?"}</DialogTitle><DialogDescription>Your Workspace Admin will review this version. You can edit it again if changes are requested.</DialogDescription></DialogHeader><div className="flex flex-wrap justify-end gap-2"><Button size="sm" variant="outline" onClick={() => setConfirmSubmit(false)}>Keep editing</Button><Button size="sm" disabled={isSending} onClick={() => sendToCommittee()}>{isSending ? "Submitting…" : "Submit for review"}</Button></div></DialogContent></Dialog>
    </div>
  );
};
