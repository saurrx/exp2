...
export const Succeeded: Story = { tags: ["viewport:1366x768", "viewport:1920x1080", "viewport:640x360@2"], parameters: state("succeeded"), play: async ({ canvasElement }) => { const d = await report(canvasElement); await expect(d.getByText("2.3")).toBeVisible(); await expect(d.getByText("Marginally novel")).toBeVisible(); } };
...
export const Error: Story = { parameters: { msw: { handlers: [route("get", "/v1/drafts/:id/evaluation", () => ({ status: 503, body: { message: "Evaluation unavailable" } }))] } }, play: async ({ canvasElement }) => { const c = await ready(canvasElement); await expect(await c.findByText("Could not load the evaluation.", {}, { timeout: 15000 })).toBeVisible(); } };
