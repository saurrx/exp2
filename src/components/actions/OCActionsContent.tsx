...
export default function OCActionsContent() {
  ...
  const query=useQuery({queryKey:["oc_action_queue",currentPage,itemsPerPage,searchQuery,filterOption,statusFilter,clientFilter,sortOption],queryFn:async()=>{
  ...
  return <ActionsWorkspace operator hasExtraFilters={statusFilter!=="all" || clientFilter!=="all"} clearExtraFilters={()=>{setStatusFilter("all");setClientFilter("all");setClientSearch("");setCurrentPage(1);}} rows={rows} loading={query.isLoading} error={query.isError} retry={()=>query.refetch()} search={searchQuery} onSearch={v=>{setSearchQuery(v);setCurrentPage(1);}} filter={filterOption} onFilter={v=>{track("list_filtered",{list:"actions"});setFilterOption(v);setCurrentPage(1);}} pagination={query.data?.pagination} onPage={setCurrentPage} extraFilters={<>
