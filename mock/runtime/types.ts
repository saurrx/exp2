...
export type Draft = { version?: number; history?: Array<{ revision: number; answers: Record<string, unknown>; submitted_at: string }>; id: string; idea_id: string; answers: Record<string, unknown>; status: "DRAFT" | "SUBMITTED"; api_evaluation_id: string | null; score: number | null; report: unknown | null; brief_summary: string | null; brief_problem: string | null; created_at: string; updated_at: string };
export type EvalState = "QUEUED" | "RUNNING" | "SUCCEEDED" | "PARTIAL" | "FAILED" | "TIMED_OUT";
export type Evaluation = { input_fingerprint?: number; input_revision?: number; id: string; draft_id: string; mode: "fixed" | "progress"; state: EvalState; final_state?: EvalState; started_at: string; score: number | null; report: unknown | null; failure_reason: string | null };
...
export type DueDate = { id: string; patent_id: string; client_id: string; event_type: string; title: string; due_at: string | null; data_issue?: string | null; source?: string; source_row?: number | null; correction_note?: string | null; updated_by?: string | null; status: DueDateStatus; created_at: string; updated_at: string };
...
export type ActionRequest = { response_note?: string | null; id: string; client_id: string; due_date_id: string; template_id: string | null; instruction: string | null; selected_countries: string[]; status: ActionStatus; submission_state: ActionSubmissionState; version: number; note: string | null; requested_by_id: string; requested_at: string; updated_at: string };
...
export type StoredFile = { content_base64?: string; idea_id?: string; id: string; client_id: string | null; original_name: string; file_name: string; content_type: string; size_bytes: number; status: FileStatus; category: string; uploaded_by_id: string; created_at: string };
