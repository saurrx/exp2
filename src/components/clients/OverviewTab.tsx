...
const OverviewTab: React.FC<OverviewTabProps> = ({ clientTeam = [], clientId, clientData, caseOwnerName, onChangeCaseOwner, canManageTeam, onEditClient }) => {
  ...
  const summaryClass = "cursor-pointer text-sm font-medium focus-visible:outline focus-visible:outline-2 focus-visible:outline-pl-brand";
  ...
  return <div className="min-w-0 text-pl-ink">
    ...
    <details className={detailClass}>
      <summary className={summaryClass}>Portfolio imports{setup?.latest_import ? ` · ${setup.latest_import.failed ? "Rows need correction" : setup.latest_import.status === "RUNNING" ? "In progress" : "Latest import complete"}` : ""}</summary>
