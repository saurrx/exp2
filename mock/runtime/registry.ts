...
export function route(method: Method, path: string, resolve: (ctx: Ctx) => Result | Promise<Result>, options: { rawResponse?: boolean | ((response: Response) => boolean) } = {}): HttpHandler {
  ...
  return http[method](path, async ({ params, request }) => {
    ...
    if (r instanceof Response && (typeof options.rawResponse === "function" ? options.rawResponse(r) : options.rawResponse)) return r;
