...
export const Completed:Story={parameters:{pulse:{scenario:"v0/due-dates/completed",route:"/due-dates?filter=completed"}},play:async({canvasElement})=>{const c=within(canvasElement);await expect(await c.findByRole("button",{name:"Reopen event…"},{timeout:15000})).toBeVisible();await expect(c.getByRole("article",{name:"Selected patent event"})).toHaveTextContent("Completed");}};
...
export const WorkspaceAdminContinuation:Story={parameters:{pulse:{scenario:"v0/actions/action-required",persona:"LEGAL_COUNSEL"}},play:async({canvasElement})=>{const c=within(canvasElement);await expect(await c.findByRole("heading",{name:"Patent event instructions"},{timeout:15000})).toBeVisible();await expect(await c.findByLabelText(/Respond to the office action/)).toBeEnabled();await expect(c.queryByRole("button",{name:"Edit event"})).toBeNull();await expect(c.queryByRole("button",{name:"Import spreadsheet"})).toBeNull();}};
