...
export default function OCActionsContent() {
  ...
  return <ActionsWorkspace operator rows={rows} loading={query.isLoading} error={query.isError} retry={()=>query.refetch()} search={searchQuery} onSearch={v=>{setSearchQuery(v);setCurrentPage(1);}} filter={filterOption} onFilter={v=>{track("list_filtered",{list:"actions"});setFilterOption(v);setCurrentPage(1);}} pagination={query.data?.pagination} onPage={setCurrentPage} extraFilters={<>
