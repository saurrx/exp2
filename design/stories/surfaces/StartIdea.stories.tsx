...
export const UnsupportedFile: Story = { play: async () => { const d = await dialog(); await userEvent.upload(d.getByLabelText("Upload source document"), new File(["synthetic"], "notes.pptx", { type: "application/octet-stream" }), { applyAccept: false }); await expect(await d.findByRole("alert")).toHaveTextContent("For slides, use"); } };
