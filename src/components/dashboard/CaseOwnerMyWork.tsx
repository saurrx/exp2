    ...
    <div data-case-owner-work className="mx-auto w-full max-w-screen-2xl px-6 pb-10 pt-4 text-pl-ink md:px-8 md:pt-6">
      ...
      {loading && !error ? <p role="status" className="py-10 text-sm text-pl-text-2">Loading your assigned-client work…</p> : error || !data ? <div className="py-8">
        ...
        <section id="assigned-clients" aria-labelledby="assigned-clients-heading" className="mt-8 border-t border-pl-border pt-5">
          ...
          {!data.clients.length ? <p className="mt-3 text-sm text-pl-text-2">No current assignments.</p> : <ul className="mt-3 divide-y divide-pl-border">{data.clients.map(client => <li key={client.client_id} className="flex flex-wrap items-center justify-between gap-3 py-3 text-sm"><div className="min-w-0"><p className="break-words font-medium">{client.client_name}</p>{!data.setup.some(item => item.kind === "setup" && item.client_id === client.client_id) && <p className="mt-1 text-xs text-pl-text-2">{client.health}{client.assigned_at && <> · Assigned {age(client.assigned_at).toLowerCase()}</>}</p>}</div><Link to={client.href} className="underline underline-offset-4">Open client workspace →</Link></li>)}</ul>}
        </section>
