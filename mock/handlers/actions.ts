...
const clientRow = (dd: ReturnType<typeof allDueDates>[number]) => {
  ...
  return {
    ...
    submission_state: a?.submission_state ?? null, status: a?.status ?? "NO_ACTION", selected_countries: a?.selected_countries ?? [], version: a?.version ?? null, note: a?.note ?? null, requested_at: a?.requested_at ?? null, event_status: dd.status, response_note: a?.response_note ?? null,
  };
};

export const actionHandlers = [
  route("get", "/v1/actions/templates", ({ url }) => (getDb().flags.v0 && currentUser()?.role === "INVENTOR" ? { status: 403, body: { message: "Actions are not part of an inventor's workspace." } } : getDb().flags.v0 ? getDb().actionTemplates.filter(t => t.enabled && t.event_types.includes(q(url,"event_type") ?? "")).sort((a,b)=>a.rank_default-b.rank_default) : templatesFor(q(url, "event_type") ?? ""))),
  route("get", "/v1/actions/queue", ({ url }) => {
    ...
    if (filter === "overdue") out = out.filter((r) => r.due_date?.status === "PENDING" && !!r.due_date.due_at && r.due_date.due_at.slice(0,10) < now.slice(0,10));
    else if (filter === "upcoming") out = out.filter((r) => r.due_date?.status === "PENDING" && !!r.due_date.due_at && r.due_date.due_at.slice(0,10) > now.slice(0,10));
    else if (filter === "dueToday") out = out.filter((r) => r.due_date?.status === "PENDING" && !!r.due_date.due_at && r.due_date.due_at.slice(0,10) === now.slice(0,10));
    ...
    return db.flags.v0 && q(url, "page_info") === "1" ? paginate(out, page, limit) : out.slice((page - 1) * limit, page * limit);
  }),
  route("get", "/v1/actions", ({ url }) => {
    ...
    let rows = allDueDates(ids).filter((d) => getDb().flags.v0 ? true : d.status === "PENDING").sort((a, b) => (a.due_at || "").localeCompare(b.due_at || ""));
    const window = q(url,"filter"), today = clock.iso().slice(0,10);
    if (window === "completed") rows = rows.filter(d => d.status === "COMPLETED");
    else if (window === "overdue") rows = rows.filter(d => d.status === "PENDING" && !!d.due_at && d.due_at.slice(0,10) < today);
    else if (window === "dueToday") rows = rows.filter(d => d.status === "PENDING" && !!d.due_at && d.due_at.slice(0,10) === today);
    else if (window === "upcoming") rows = rows.filter(d => d.status === "PENDING" && !!d.due_at && d.due_at.slice(0,10) > today);
    const search = (q(url, "search") ?? "").trim().toLowerCase();
  ...
  route("post", "/v1/actions/submit-all", async ({ body }) => {
    const db = getDb(); const u = currentUser();
    if (!u || u.role !== "LEGAL_COUNSEL") return { status: 403, body: { message: "Only in-house counsel can instruct Photon Legal." } };
    const b = await body() as { action_ids?: string[] };
    const selected = db.flags.v0 && Array.isArray(b.action_ids) ? b.action_ids : null;
    if (selected && selected.some(id => !db.actionRequests.some(a => a.id === id && a.client_id === u.client_id))) return { status: 404, body: { message: "Instruction not found." } };
    let n = 0;
    for (const a of db.actionRequests) if (a.client_id === u.client_id && a.submission_state === "DRAFT" && (!selected || selected.includes(a.id))) { a.submission_state = "SUBMITTED"; a.status = "NEW"; a.requested_at = clock.iso(); a.updated_at = clock.iso(); n++; }
    touched();
    return { submitted: n };
  }),
  route("patch", "/v1/actions/:id", async ({ params, body }) => {
    ...
    if (db.flags.v0 && (u.role !== "LEGAL_COUNSEL" || u.client_id !== dd.client_id)) return { status: 403, body: { message: "Only the client's Workspace Admin can send an instruction." } };
    if (db.flags.v0 && dd.status !== "PENDING") return { status: 409, body: { message: "This event is already complete." } };
    const t = db.actionTemplates.find((x) => x.id === b.template_id);
    if (db.flags.v0 && (!t || !t.enabled || !t.event_types.includes(dd.event_type))) return { status: 400, body: { message: "Choose an available instruction for this event." } };
    ...
    else if (a.submission_state !== "DRAFT") { a.submission_state = "UPDATED"; a.status = "NEW"; a.response_note = null; a.version += 1; a.requested_at = clock.iso(); a.requested_by_id = u.id; }
    a.template_id = t?.id ?? null; a.instruction = db.flags.v0 ? t?.label ?? null : b.instruction ?? t?.label ?? null; a.selected_countries = b.selected_countries ?? []; a.note = b.note ?? null; a.updated_at = clock.iso();
    touched();
    return a;
  }),
  route("patch", "/v1/actions/:id/request-status", async ({ params, body }) => {
    ...
    if (!u || !["CASE_OWNER", "PHOTON_ADMIN", "PHOTON_SUPERADMIN"].includes(u.role)) return { status: 403, body: { message: "Photon Legal only." } };
    const scope = scopeFor(u);
    if (db.flags.v0 && (scope !== null && !scope.includes(a.client_id))) return { status: 404, body: { message: "Instruction not found." } };
    if (db.flags.v0 && (a.submission_state === "DRAFT" || ["COMPLETED","DECLINED"].includes(a.status))) return { status: 409, body: { message: "This request cannot advance." } };
    ...
    if (db.flags.v0 && order.indexOf(next) !== order.indexOf(a.status) + 1) return { status: 409, body: { message: "Use the next operational step." } };
  ...
  route("post", "/v1/actions/:id/resolve", async ({ params, body }) => {
    ...
    const a = db.actionRequests.find((x) => x.id === params.id);
    if (!a) return { status: 404, body: { message: "Action not found." } };
    if (db.flags.v0) { const u = currentUser(); if (!u || !["CASE_OWNER","PHOTON_ADMIN"].includes(u.role)) return { status: 403, body: { message: "Photon Legal only." } }; const scope = scopeFor(u); if (scope !== null && !scope.includes(a.client_id)) return { status: 404, body: { message: "Instruction not found." } }; if (a.submission_state === "DRAFT" || ["COMPLETED","DECLINED"].includes(a.status)) return { status: 409, body: { message: "This request is not available for processing." } }; if (b.status !== "DECLINED") return { status: 400, body: { message: "Use the next operational step to complete a request." } }; }
    if (b.status === "DECLINED" && !b.note?.trim()) return { status: 400, body: { message: "A note is required to decline." } };
    a.status = b.status === "DECLINED" ? "DECLINED" : "COMPLETED"; if (db.flags.v0) a.response_note = b.note ?? null; else a.note = b.note ?? a.note; a.updated_at = clock.iso(); touched();
    return a;
  }),
];
