...
export const actionHandlers = [
  ...
  route("get", "/v1/actions/queue", ({ url }) => {
    ...
    else if (filter === "dueToday") out = out.filter((r) => r.due_date?.status === "PENDING" && r.due_date.due_at.slice(0,10) === now.slice(0,10));
  ...
  route("get", "/v1/actions", ({ url }) => {
    ...
    else if (window === "overdue") rows = rows.filter(d => d.status === "PENDING" && d.due_at.slice(0,10) < today);
    else if (window === "dueToday") rows = rows.filter(d => d.status === "PENDING" && d.due_at.slice(0,10) === today);
    else if (window === "upcoming") rows = rows.filter(d => d.status === "PENDING" && d.due_at.slice(0,10) > today);
    const search = (q(url, "search") ?? "").trim().toLowerCase();
