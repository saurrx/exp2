...
export const RequestChanges: Story = { play: async ({ canvasElement }) => { const d = await confirm(canvasElement, "request"); await expect(d.getByRole("button", { name: "Send request" })).toBeDisabled(); await userEvent.click(d.getByRole("textbox")); await userEvent.paste(reason); } };
export const Reject: Story = { play: async ({ canvasElement }) => { const d = await confirm(canvasElement, "reject"); await expect(d.getByRole("button", { name: "Reject idea" })).toBeDisabled(); await userEvent.click(d.getByRole("textbox")); await userEvent.paste(reason); } };
